...
class MaskedAutoencoderViT(nn.Module):
    ...
    def adain(self, contents, style_mean, style_std, device):
        content_means, content_stds = torch.mean(contents, dim=[2, 3], keepdim=True), torch.std(contents, dim=[2, 3], keepdim=True)

        style_mean = style_mean.to(device, non_blocking=True)
        style_std = style_std.to(device, non_blocking=True)

        normalized_contents = (contents - content_means) / content_stds
        stylized_contents = normalized_contents * style_std + style_mean
        return stylized_contents

    def normalize_preprocess_image(self, image):
        transform = transforms.Compose([
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ])
        return transform(image)

    def forward(self, imgs, device, epoch, mask_ratio=0.75, heatmaps=None, args=None):
 
        if args.adain == True:
            imgs = imgs * torch.tensor(self.std).to(device, non_blocking=True).view(1, -1, 1, 1) + torch.tensor(self.mean).to(device, non_blocking=True).view(1, -1, 1, 1)
            adain_imgs = self.adain(imgs, self.style_mean, self.style_std, device)
            adain_imgs = self.normalize_preprocess_image(adain_imgs)
            adain_imgs = adain_imgs.to(device, non_blocking=True)
            latent, mask, ids_restore = self.forward_encoder(adain_imgs, mask_ratio)
            pred = self.forward_decoder(latent, ids_restore)
            loss = self.forward_loss(adain_imgs, pred, mask)
